...
def plot_counts(df, outfile, cell_line_col="cell_line", gene_col="gene", seed_col="transfer_split_seed1"):
    """
...
def create_dataset_variants(adata, balanced_transfer_splitter, base_fractions=[1.0, 0.6, 0.2]):
    ...
    all_perturbations_to_remove = ["C7orf26", "CPSF3", "FAM136A", "THAP1", "TRNT1", "MRPL35"]
    #some_perturbations_to_remove = ["XPO7", "RB1CC1", "SATB2"]
    some_perturbations_to_remove = ["C7orf26", "CPSF3", "FAM136A"]
    ...
    for config_name, config in dataset_configs.items():
        ...
        if config['add_back_as_train']:
            ...
            rows_to_add = adata.obs[adata.obs["gene"].isin(config['add_back_as_train'])].copy()
...
hydra:
    run:
        dir: ${{paths.log_dir}}/${{task_name}}/runs/${{now:%Y-%m-%d}}_${{now:%H-%M-%S}}_replogle_{dataset_name}
    sweep:
            dir: ${{paths.log_dir}}/${{task_name}}/multiruns/${{now:%Y-%m-%d}}_${{now:%H-%M-%S}}
            subdir: ${{hydra.job.num}}_replogle_{dataset_name}
...
def save_datasets(datasets, adata, output_dir="/gpfs/home/asun/jin_lab/perturbench/0_datasets/clean/", 
    ...
    for dataset_name, df in datasets.items():
        ...
        # Plot split
        fig_filename = f"replogle_df_{dataset_name}_train_downsample_only_split.png"
...
if __name__ == "__main__":
    ...
    adata = sc.read_h5ad('/gpfs/home/asun/jin_lab/perturbench/0_datasets/replogle_even_split.h5ad')
    
    # Create initial filtered dataset for splitter
    #perturbations_to_remove = ["XPO7", "RB1CC1", "SATB2", "CX3CL1", "CUL1", "TBR1"]
    perturbations_to_remove = ["C7orf26", "CPSF3", "FAM136A", "THAP1", "TRNT1", "MRPL35"]
    df_100_5_initial = adata.obs[~adata.obs["gene"].isin(perturbations_to_remove)]
    ...
    save_datasets(
        datasets, 
        adata, 
        output_dir="/gpfs/home/asun/jin_lab/perturbench/0_datasets/replogle/data/",
        csv_dir="/gpfs/home/asun/jin_lab/perturbench/0_datasets/replogle/splits/",
        fig_dir="/gpfs/home/asun/jin_lab/perturbench/0_datasets/replogle/figures/",
        yaml_dir="/gpfs/home/asun/jin_lab/perturbench/src/perturbench/src/perturbench/configs/experiment/replogle/"
    )
